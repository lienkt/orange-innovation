...
doc.cover(
    ...
    statement="Two runtimes over one SQLite file. A batch discovery pipeline of thirteen stages writes; a read-mostly "
              "FastAPI service serves the read model and the built React bundle from the same origin. Nothing in the "
              "serving path imports the pipeline's heavy dependencies, no language model writes to the database "
              "unvalidated, and every stored number carries the inputs and the configuration version that produced it.")
...
doc.table(
    ...
    [["Language (backend)", "Python", "≥ 3.10, deployed on 3.13", "Ecosystem for the statistical and NLP work; one "
     ...
     ["Tests", "pytest", "≥ 8.0", "167 tests; the model provider has a mock implementation so no test needs a network"]],
...
doc.table(
    ...
    [["`config.py`", "553", "Controlled vocabularies, crosswalks, business graph configuration, validation at load"],
     ...
     ["`api.py`", "1,196", "FastAPI application, 46 endpoints, static mount for the built frontend"],
     ["`cli.py`", "546", "Command line: check, refresh, serve, topics, show, whitespace, replay, brief, the three "
      "competitor commands, internal intake, and the rest"]],
...
doc.table(
    ...
    [["`raw_items`", "11,353", "Replay archive — the connector payload as returned", "Kept locally; dropped from the "
     ...
     ["`refreshes`", "34", "One row per run, with reference date, replay flag and per-stage statistics", "Indefinite"],
     ...
     ["`workflow_transitions`", "1", "Full stage history with actor and reason", "Indefinite"],
     ...
     ["`market_sizes`", "313", "TAM/SAM/SOM by method, with every factor and its source", "Full history"],
...
doc.p("Forty-six endpoints on one FastAPI application. The read endpoints are the product; the write "
      "endpoints exist for curation, collaboration and on-demand generation.")
doc.table(
...
doc.callout("Changing any weight requires a new weight-set identifier",
            ["Scores across a version boundary are not comparable. Every score records the set that produced it, and "
             ...
             "`sizing_version` on every market size and `register_version` on every competitive assessment."],
            SH_RED, RED)
...
doc.h2("16.1  Current posture — read this before any wider use")
doc.callout("The deployed demonstration is public and unauthenticated", [
    "That is acceptable for a demonstration and is not acceptable for anything else. Two consequences follow:",
    "The brief PDFs are stamped *Internal*, and the business graph, the competitor register and the reference density "
    "are Orange's own material. Anyone with the URL can read all of it.",
    "`POST /api/topics/{id}/description` and `POST /api/topics/{id}/brief` call the configured model with the deployed "
    "key, so anyone with the URL can spend it. That key was shared in plaintext during development and should be "
    "rotated regardless, with a separate key issued for CI.",
    "Either an IP access restriction or a Microsoft Entra sign-in requirement closes both, in one command each.",
], SH_RED, RED)
...
doc.p("**Not yet present, and required before production:** authentication, per-role authorisation on the write "
      "endpoints, rate limiting on the two generation endpoints, an audit log distinct from the workflow transition "
      "history, and a documented key-rotation procedure.")

# ============================================================ 17
doc.h1("17   Testing strategy")
doc.p("294 tests across fourteen modules. They cover the invariants that would be expensive to discover late rather than "
      "aiming at a coverage percentage, and the model provider's mock implementation means none of them touches a network.")
doc.table(
    ["Suite", "What it holds"],
