...
| `refreshes` | 34 | One row per run: reference date, replay flag, per-stage statistics, per-source errors. |
...
| `workflow_transitions` | 1 | Full stage history with actor, role and reason. |
